...
const SingleBookBlock = ( { book, ...props } ) => {
	const addToCart = () => {
		const res = props.addToCartAction(book._id);
		...
		<div className="card">
			<div className="card-image">
				<div className="book-cover text-center">
					{(book.book_cover) 
					? <img height="250" src={book.book_cover} alt={book.book_title} />
			...
			<div className="card-header">
				<div className="book-title text-center d-flex align-items-center justify-content-between py-2">
					<h5 className="mb-0">{book.book_title}</h5>
					<p className="mb-0"><strong>${book.book_price}</strong></p>
				</div>
			</div>
			<div className="card-body">
				{book.book_description && book.book_description.substr(1, 100) + '...'}
			</div>
			<div className="card-footer d-flex align-items-center justify-content-between">
				<button className="btn btn-primary" data-bookid={book._id} onClick={addToCart}>Add to cart</button>
				<Link to={`/products/${book.book_isbn}`} state={{modal: true, returnTo: location.pathname}}>More info</Link>
			</div>
		</div>
	);
}
